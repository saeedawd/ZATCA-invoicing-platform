<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <style>
        @include('pdf.templates.partials.base-styles')
        @page { margin: 14mm 12mm; }

        .top-rule {
            height: 1px;
            background: {{ $brandSecondary }};
            margin-bottom: 14px;
        }
        .brand-name {
            color: {{ $brandPrimary }};
            font-size: 16px;
            font-weight: bold;
        }
        .meta td { padding: 2px 0; font-size: 11px; }
        .meta .label { color: #6b7280; width: 36%; }
        .meta .value { font-weight: bold; width: 64%; }

        .box {
            border: 1px solid #e5e7eb;
            padding: 10px 12px;
            background: #fff;
        }
        .box-title {
            color: {{ $brandPrimary }};
            font-weight: bold;
            font-size: 11px;
            margin-bottom: 6px;
            padding-bottom: 4px;
            border-bottom: 1px solid #f3f4f6;
        }

        .items thead th {
            background: #fff;
            color: {{ $brandPrimary }};
            font-size: 10px;
            padding: 8px 5px;
            border-top: 1px solid {{ $brandPrimary }};
            border-bottom: 1px solid {{ $brandPrimary }};
            border-left: none;
            border-right: none;
        }
        .items tbody td {
            border-bottom: 1px solid #eceff3;
            padding: 8px 5px;
            font-size: 10px;
        }

        .totals td {
            padding: 7px 10px;
            font-size: 11px;
            border-bottom: 1px solid #f3f4f6;
        }
        .totals .totals-head td {
            background: #f9fafb;
            color: {{ $brandPrimary }};
            font-weight: bold;
            font-size: 11px;
            border-bottom: 1px solid #e5e7eb;
            padding: 8px 10px;
        }
        .totals .totals-row .label { color: #6b7280; }
        .totals .totals-row .amount { font-weight: bold; color: #111827; }
        .totals .payable td {
            background: {{ $brandPrimary }};
            color: #fff;
            font-weight: bold;
            font-size: 12px;
            border: none;
            padding: 9px 10px;
        }
        .totals .amount-paid td {
            background: #ecfdf5;
            color: #047857;
            font-weight: bold;
            border-bottom: 1px solid #d1fae5;
            padding: 8px 10px;
        }
        .totals .balance-due td {
            background: #fff7ed;
            color: #c2410c;
            font-weight: bold;
            border: none;
            padding: 9px 10px;
        }

        .qr-wrap {
            border: 1px solid #e5e7eb;
            background: #fafafa;
            padding: 10px;
            text-align: center;
        }
        .qr-wrap img { width: 100px; height: 100px; }

        .footer {
            margin-top: 12px;
            border-top: 1px solid #e5e7eb;
            padding-top: 6px;
            color: #6b7280;
            font-size: 8px;
        }

        .doc-title { color: {{ $brandPrimary }}; }
    </style>
</head>
<body>
    <div class="top-rule"></div>
    <table class="ar" style="margin-bottom:14px;">
        <tr>
            <td width="70%" class="ar middle">
                <div class="brand-name">{{ $sellerNameAr }}</div>
                <div class="small muted" style="margin-top:4px;">الرقم الضريبي: {{ $organization?->vat_number ?: '—' }}
                    @if ($organization?->cr_number)
                        | السجل: {{ $organization->cr_number }}
                    @endif
                </div>
                <div class="small muted">{{ $sellerAddressAr }}</div>
            </td>
            <td width="30%" class="center middle">
                @if ($logoDataUri)
                    <img class="logo-img" src="{{ $logoDataUri }}" alt="logo">
                @endif
            </td>
        </tr>
    </table>

    <table class="ar" style="margin-bottom:12px;">
        <tr>
            <td width="58%" class="middle">
                <div class="h2 doc-title">{{ $documentTitleAr }}</div>
                <div class="gap-sm"></div>
                <table class="meta">
                    <tr>
                        <td class="label">{{ $documentNumberLabel ?? 'رقم الفاتورة' }}</td>
                        <td class="value" style="text-align:right;">{{ $invoice->invoice_number ?: '—' }}</td>
                    </tr>
                    <tr>
                        <td class="label">التاريخ</td>
                        <td class="value" style="text-align:right;">{{ optional($invoice->issue_date)->format('Y-m-d') }}</td>
                    </tr>
                    <tr>
                        <td class="label">{{ $documentExtraLabel ?? 'الوقت' }}</td>
                        <td class="value" style="text-align:right;">{{ $documentExtraValue ?? ($invoice->issue_time ?: '—') }}</td>
                    </tr>
                </table>
            </td>
            <td width="42%">
                <div class="box ar">
                    <div class="box-title">المشتري</div>
                    <div class="bold" style="font-size:12px;">{{ $invoice->party?->name ?: 'عميل نقدي' }}</div>
                    @if ($invoice->party?->vat_number)
                        <div class="small muted" style="margin-top:4px;">الرقم الضريبي: {{ $invoice->party->vat_number }}</div>
                    @endif
                    @if ($buyerAddress !== '—')
                        <div class="small muted">{{ $buyerAddress }}</div>
                    @endif
                </div>
            </td>
        </tr>
    </table>

    @include('pdf.templates.partials.items')
    <div class="gap"></div>
    @include('pdf.templates.partials.totals', ['borderColor' => '#e5e7eb'])
    <div class="gap"></div>
    @include('pdf.templates.partials.qr-footer')
</body>
</html>
